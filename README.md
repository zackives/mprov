...
This Python package connects to the [PennProvenance](https://www.pennprovenance.net) provenance services.
...
conn = MProvConnection('YOUR_USERNAME', 'YOUR_PASSWORD', 'localhost:8088')

# Name the graph for provenance
conn.set_graph('Prov1')

# Create a simple relation or stream, with a binary schema
data_schema = BasicSchema('SampleStream', {'key': 'int', 'value': 'int'})
# Create a sample tuple
tuple = data_schema.create_tuple({'key': 1, 'value': 456})
...
out_tuple = data_schema.create_tuple(result)

# Store the derived tuple and the derivation name / time
conn.store_derived_result('OutStream', out_tuple['key'], out_tuple, input_token, 'area_circle', 1, )
